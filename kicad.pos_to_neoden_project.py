...
def transform_pos_rotation(rotation, side):
    angle = float(rotation) % 360.0
    if str(side or "").strip().lower() == "bottom":
        angle = (360.0 - angle) % 360.0
    return transrotate(angle)
...
def process_pos_lines(
    ...
    for line in pos_lines:
        ...
        x = x_val
        y = y_val
        rotation = transform_pos_rotation(line[5], line[-1] if line else "")
